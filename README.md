...
Pull requests run hosted contract and all six managed architecture/flavor tests, build and statically validate all six MSI variants, exercise the x86, x64, and ARM64 native hosts' pre-CLR self-tests, and perform architecture-matched MSI install/ACL/repair/uninstall smoke tests on guarded ephemeral Windows runners. They never execute branch code on protected SDK hosts. A release fails closed unless the exact current `main` commit has successful CI and Hosted WireSock SDK experiment push runs; the release workflow independently verifies both results before building any public artifact. Optional connected-state SDK validation remains available for installations that can supply protected self-hosted runners and profiles. That privileged workflow is loaded from one audited full commit SHA, derives and validates the exact caller commit/ref, and exercises transparent and virtual-adapter lifecycle, network-lock enable/reset behavior, and a complete Amnezia 2.0 profile. Protect the `wiresock-sdk` environment with required reviewers. If these optional pools are deployed, place disposable, just-in-time x86-workload and x64-workload pools on x64 hosts plus an ARM64-workload pool on ARM64 hosts in an organization runner group named `wiresock-sdk`. Give them the exact logical routing labels `wiresock-sdk-x86`, `wiresock-sdk-x64`, and `wiresock-sdk-arm64`, restrict that group's workflow access to only the exact `wiresock/WireSockUI/.github/workflows/sdk-integration.yml@0440f3d3a42216a23ce455686ce983b88af62a3e` selected workflow used by the callers, and retain the common `wiresock-sdk` runner label. Set `WIRESOCKUI_SDK_INTEGRATION_ENABLED=true` only after all three disposable runner pools and the selected-workflow runner-group policy are available; otherwise protected `main`, scheduled, and release runs skip this optional gate.
...
WireSock UI is published from the canonical personal-account repository `wiresock/WireSockUI`. Because that repository has only its owner as a collaborator, neither `main` nor the `release-publish` environment pretends to provide an independent reviewer gate. Main must require pull requests, dismiss stale reviews, require resolved conversations, enforce administrators, require signed commits, and require the exact CI checks, but its approval count and latest-push-approval option must remain zero/disabled until an independent collaborator exists. The publication environment must have no required reviewers and must allow exactly the custom tag policy `release-v*.*.*`. Release authorization instead combines a GitHub-verified signed annotated tag at the exact gated `main` tip with repository rules that let only the `wiresock` user create an active release tag and let nobody update or delete it. The release preflight also requires canonical personal ownership and enabled immutable releases. Configure repository Actions policy to require full-SHA action pinning, remove the retired Azure signing actions from the allowlist, and limit allowed actions to the audited set. The required checks must include the dependency audit, SDK contract, all six managed architecture/flavor test variants, all six publish checks, all three native-host smoke variants, all three architecture-matched MSI install smoke variants, architecture isolation, and transition smoke, bound to the GitHub Actions App rather than accepting same-named third-party statuses. Repository rules must restrict creation, update, and deletion of the active `release-v*.*.*` namespace and the retired `v*` namespace.
...
As a mandatory external repository control, every repository ruleset contributing to `main` or to tag update/deletion restrictions must have an empty bypass-actor list. Put active `release-v*.*.*` creation in a separate creation-only ruleset whose bypass list contains only the `wiresock` user; never grant broad repository roles, Apps, or deploy keys this bypass. Keep update and deletion in a separate zero-bypass ruleset so even the owner cannot move or delete a created tag. The retired `v*` namespace must have zero-bypass creation/update/deletion rules to keep it permanently retired. The read-only checker requires exactly these three active repository tag rulesets, their exact wildcard coverage, and the separation of creation from mutation rules. GitHub withholds bypass actors from a token without ruleset write access, so the checker validates them when GitHub exposes them and otherwise leaves their exact contents as an independently audited configuration requirement. Do not grant the policy-reader App Administration (write) merely to inspect them, because that would turn a read-only release gate into a repository-takeover credential. Classic branch-protection PR bypass allowances are visible to the read-only checker and must remain empty.

Release tags remain signed annotated Git metadata in strict `release-vMAJOR.MINOR.PATCH` form, resolve directly to the current protected `main` tip, and have a GitHub-verified cryptographic signature. This tag signature authorizes the source revision; it does not sign any binary or MSI. Create tags with `git tag -s release-vMAJOR.MINOR.PATCH`; lightweight, unsigned, stale-main, moved, or legacy `v*` tags are rejected. The workflow binds the tag object's internal name and exact object ID during authorization, then revalidates both immediately before publication and after publication. Rotate or delete legacy release PAT/PFX and Azure signing secrets (including `MY_GITHUB_PAT` and all `AZURE_ARTIFACT_SIGNING_*` values). Publication uses only the repository `GITHUB_TOKEN`; the read-only policy App can inspect controls but cannot publish or change repository settings. Until the protected publication environment, personal-repository rules, OIDC claim template, exact action pins, and immutable-release setting are configured, release execution is intentionally unavailable rather than silently weakening these controls. The optional real-SDK integration remains unavailable until its organization runner group and protected runner pools exist.
